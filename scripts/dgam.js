import data from '../data/dgam.json' with { type: 'json' };

const parametros = data.DGAM;
const monitor = document.getElementById('mission-monitor');
let status = "Incompleta";
const reportes = localStorage.getItem('dgam') ? JSON.parse(localStorage.getItem('dgam')) : [];

const renderExperiment = () => {
    monitor.innerHTML = `
        <h1>Bienvenidos a la División de Geología y Aprovechamiento de Minerales - DGAM</h1>
        ...
        <form id="experiment-form">
            <label for="flujo">Flujo (mL/min)</label>
            <input type="number" id="flujo" name="flow_ml_min" placeholder="Flujo del líquido" min="0" value="0">
            ...
            <label for="ultrasonic">Agitación Ultrasónica (min)</label>
            <input type="number" id="ultrasonic" name="ultrasonic_min" placeholder="Tiempo de agitación" min="0" value="0">
    ...
    experimentForm.addEventListener('submit', (e) => {
        ...
        localStorage.setItem('dgam', JSON.stringify(reportes));
    });
};
